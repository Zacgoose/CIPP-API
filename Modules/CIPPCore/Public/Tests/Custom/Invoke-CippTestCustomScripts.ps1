        ...
        foreach ($Script in $LatestScripts) {
            ...
            try {
                ...
                if ($FailedRows.Count -gt 0) {
                    $ResultMarkdown = "Custom script returned failure result:`n$($FailedRows | ConvertTo-Json -Depth 10)"
                    Add-CippTestResult -TenantFilter $Tenant -TestId $TestId -TestType 'Custom' -Status 'Failed' -ResultMarkdown $ResultMarkdown -Risk ($Script.Risk ?? 'Medium') -Name $ScriptName -Pillar $Script.Pillar -UserImpact $Script.UserImpact -ImplementationEffort $Script.ImplementationEffort -Category 'Custom Script'
                    if ($ShouldAlert) {
                        Write-AlertMessage -tenant $Tenant -message "Custom script test failed: $ScriptName ($($Script.ScriptGuid))"
                    }
                } else {
                    Add-CippTestResult -TenantFilter $Tenant -TestId $TestId -TestType 'Custom' -Status 'Passed' -ResultMarkdown 'Custom script test passed with no failures.' -Risk ($Script.Risk ?? 'Medium') -Name $ScriptName -Pillar $Script.Pillar -UserImpact $Script.UserImpact -ImplementationEffort $Script.ImplementationEffort -Category 'Custom Script'
                }
            } catch {
