function Test-CustomScriptSecurity {
    ...
    # Require explicit return statements so output is intentionally produced
    $ReturnStatements = $Ast.FindAll({
        param($node)
        $node -is [System.Management.Automation.Language.ReturnStatementAst]
    }, $true)

    if (-not $ReturnStatements -or $ReturnStatements.Count -eq 0) {
        throw 'Custom scripts must include at least one explicit return statement. Scripts without an explicit return are treated as no-output scripts.'
    }
