function New-ExoRequest {
    ...
    if ((Get-AuthorisedRequest -TenantID $tenantid) -or $NoAuthCheck -eq $True) {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'AvailableCmdlets') {
            ...
            return (Invoke-CIPPRestMethod -Uri $URL -Headers $Headers -UseLegacyInvokeRestMethod).value.exportedCmdlets -split ',' | Where-Object { $_ } | Sort-Object
        }
        ...
        if ($PSCmdlet.ParameterSetName -eq 'ExoRequest') {
            try {
                ...
                $ReturnedData = do {
                    ...
                    $Return = Invoke-CIPPRestMethod @ExoRequestParams -ResponseHeadersVariable ResponseHeaders -UseLegacyInvokeRestMethod
