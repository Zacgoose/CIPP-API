...
if (-not $OutputPath) {
    $defaultLibData = Join-Path $ModulePath 'lib' 'data' 'function-parameters.json'
    $OutputPath = if (Test-Path (Split-Path -Parent $defaultLibData)) { $defaultLibData } else { Join-Path $ModulePath 'function-parameters.json' }
}
...
$json = $functionParameters | ConvertTo-Json -Depth 8
