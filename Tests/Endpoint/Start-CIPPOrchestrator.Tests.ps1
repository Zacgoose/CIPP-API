...
Describe 'Start-CIPPOrchestrator' {
    ...
    It 'stores and queues input when called without CallerIsQueueTrigger' {
        ...
        Mock -CommandName Start-NewOrchestration -MockWith {
            param($FunctionName, $InputObject)
            $script:startCount++
            $script:lastStartInput = $InputObject
            'instance-id'
        }
    ...
    It 'starts orchestration directly when explicitly called by queue trigger' {
        ...
        $script:lastStartInput | Should -Match '"OrchestratorName":"OnboardingOrchestrator"'
        $script:lastStartInput | Should -Match '"FunctionName":"ExecOnboardTenantQueue"'
