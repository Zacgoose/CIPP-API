...
- **ScheduledTask** - Alerts from scheduled tasks (when command doesn't match other categories)

### Alert Sources

The standardized schema applies to:
- **Notification Alerts** - Alerts generated by `Push-SchedulerCIPPNotifications` from the CIPP logbook
- **Scheduled Task Webhooks** - Alerts from scheduled tasks with webhook post-execution (e.g., MFA checks, license monitoring)
- **Standards Alerts** - Standards drift detection alerts

All webhook alert types respect the `UseStandardizedSchema` configuration setting.
